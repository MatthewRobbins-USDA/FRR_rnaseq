...
cwd = os.getcwd()
cols_indx = []
helpScreen = """Usage: python3 csv_add_annotations.py [OPTION] csv_file/tsv_file csv_annotation_files
...
def gff_parsing(csvFile,annotations):
    ...
    for i in annotations:
        annot_parse = i.split('\t')
        if annot_parse[0] in annot_dict:
            annot_dict[annot_parse[0]] += ";{0}".format(annot_parse[3])
        else:
            annot_dict[annot_parse[0]] = annot_parse[3]
    ind = 1
    for n,i in enumerate(csv[0]):
        if i[:3] == "V3.":
            ind = n
            break
    print(csv[0])
    print(csv[0][ind][:-2])
    for i in csv:
        if i[ind][:-2] in annot_dict:
            i.append("\""+annot_dict[i[ind][:-2]]+"\"")
        else:
            i.append("")
    
    outFile = open(os.path.join(cwd,"interpro_"+"".join(csvFile.split('.')[:-1])+".csv"),"w")
    for l in csv:
